...
class GetDCT_resize(object):
    def __init__(self, dct_filter_size = 8, resize_ratio = 1.0):
        self.jpeg_encoder = TurboJPEG()
    ...
    def __call__(self,img):
        img = F.resize(img, int(img.shape[-2]*self.resize_ratio), self.interpolation) 
...
class SubsetDCT(object):
    def __init__(self, channels=24):
        ...
        if channels not in [192, 768,108, 432, 300]:
    ...
    def __call__(self, tensor):
        ...
        if self.channels not in [192, 768, 108, 432, 300]:
