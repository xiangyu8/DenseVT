...
_C.DATA.DCT_STATUS = False
_C.DATA.DCT_WINDOW = 8
_C.DATA.OVERLAP = 0.0
_C.DATA.RESIZE_RATIO = 1.0
_C.DATA.CHANNELS = 0
...
def update_config(config, args):
    ...
    if args.dct_status:
        config.DATA.DCT_STATUS = args.dct_status
    if args.channels:
        config.DATA.CHANNELS = args.channels
    if args.dct_window:
        config.DATA.DCT_WINDOW = args.dct_window
    if args.overlap:
        config.DATA.OVERLAP = args.overlap
    if args.resize_ratio:
        config.DATA.RESIZE_RATIO = args.resize_ratio
