...
class ResNet(nn.Module):
    maml = False #Default
    def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten = True, pre_indim = None, dct_status = False, subset = None, num_classes = 1000):
        ...
        if dct_status:
            if subset is not None:
                ...
                indim = 64
                assert pre_indim is not None and pre_indim!= indim, "The pre_indim should be different from indim."
                conv_preprocess = nn.Conv2d(pre_indim, indim, kernel_size=1,  bias=False)
                trunk = [conv_preprocess]
        else:
        ...
        self.fc = distLinear(list_of_out_dims[-1], num_classes) 
...
def ResNet10_dct_subset( flatten = True, pre_indim = None, dct_status = True, subset = 24):
    return ResNet(SimpleBlock, [1,1,1,1],[64,128,256,512], flatten, pre_indim, dct_status, subset )

def ResNet10_dct_bridge( flatten = True, pre_indim = 192, dct_status = True, subset = None, num_classes = 1000):
    return ResNet(SimpleBlock, [1,1,1,1],[64,128,256,512], flatten, pre_indim, dct_status, subset, num_classes = num_classes )

def ResNet18_dct_bridge( flatten = True, pre_indim = 192, dct_status = True, subset = None, num_classes = 1000):
    return ResNet(SimpleBlock, [2,2,2,2],[64,128,256,512], flatten, pre_indim, dct_status, subset, num_classes = num_classes )

def ResNet50_dct_bridge( flatten = True, pre_indim = 192, dct_status = True, subset = None, num_classes = 1000):
    return ResNet(BottleneckBlock, [3,4,6,3], [256,512,1024,2048], flatten, pre_indim, dct_status, subset, num_classes = num_classes )

"""
def ResNet18( flatten = True):
    return ResNet(SimpleBlock, [2,2,2,2],[64,128,256,512], flatten)

def ResNet18_dct( flatten = True):
    return ResNet_dct(SimpleBlock, [2,2,2,2],[64,128,256,512], flatten)

def ResNet34( flatten = True):
    return ResNet(SimpleBlock, [3,4,6,3],[64,128,256,512], flatten)

def ResNet34_dct( flatten = True):
