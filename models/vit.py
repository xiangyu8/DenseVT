...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm,dct_status = False,inputmix = False,  **kwargs):
        super().__init__()
...
#        self.fc = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.fc = distLinear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
...
def deit_small(patch_size=16, img_size=[224], pretrained=False, num_classes = 0, dct_status = False, in_chans = 3,**kwargs):
    model = VisionTransformer(img_size=img_size,num_classes = num_classes,
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), dct_status = dct_status, in_chans = in_chans,**kwargs)
    if pretrained:
        checkpoint = download_weights("deit_small", patch_size, kwargs['pretrained_type'])
        model.load_state_dict(checkpoint)
    return model

def deit_small_mix(patch_size=16, img_size=[224], pretrained=False, num_classes = 0, dct_status = False, in_chans = 3,**kwargs):
    model = VisionTransformer(img_size=img_size,num_classes = num_classes,
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), dct_status = dct_status, in_chans = in_chans,inputmix = True,**kwargs)
    if pretrained:
        checkpoint = download_weights("deit_small", patch_size, kwargs['pretrained_type'])
        model.load_state_dict(checkpoint)
    return model
